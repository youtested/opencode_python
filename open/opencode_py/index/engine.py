...
def _fmt_def(s: Symbol, root: Path) -> str:
    ...
    kind = s.kind
    return (f"{rel}:{s.line}\n"
            f"  {sig}  ({kind} in {where or 'module'})")


def _fmt_ref(r: Ref, root: Path) -> str:
    return f"{r.file}:{r.line}"
    #  ({r.role} in {r.container or 'module'})")


def _group_files(items: list[tuple[str, object, int]]) -> list[str]:
    groups: dict[str, list[str]] = {}
    order: list[str] = []
    for file_key, obj, rank in items:
...
def _run_query(files: dict[str, FileIndex], mode: str, term: str,
    ...
    if mode == "def":
        ...
        if mode == "callers":
            # fell through: same as refs but call-only already
            pass
    ...
    if not matches:
        return {
            ...
            "metadata": {"term": term, "mode": mode, "matches": []},
        }
